...
-- ------------------------------------------------------------
-- INTRO SOUND (only if enabled) â€“ now uses new filename and always redownloads
-- ------------------------------------------------------------
local introSoundInstance = nil
if introSoundEnabled then
    local urlIntro = "https://files.catbox.moe/hg5cr4.mp3"   -- <-- this is the one you want
    local numeFisier = "movee_intro.mp3"                     -- <-- NEW FILENAME to avoid old cache

    -- Always download and overwrite the file (we use pcall to avoid errors)
...
local ragdollGuiEnabled=true   -- always show ragdoll timer gui by default
local persistentRagdollGui=nil -- reference to a persistent "always on" display
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    -- Outer card â€” dark, thick pink border, matches photo
    ...
    -- Title (white bold, centered, photo style â€” no colored bar)
...
startBatAimbot=function()
    ...
    local AIMBOT_SPEED=62;local MELEE_OFFSET=3;local velHistory={};local MAX_HIST=6;local lastTargetPos=nil
    ...
    aimbotConn=RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            if not _predBall then
                ...
                _predBall.Color=Color3.fromRGB(255,0,255)
        ...
        if moveDir.Magnitude>1 then
            hrp.AssemblyLinearVelocity=moveDir.Unit*AIMBOT_SPEED
        else
...
local function buildMobileButtons()
    ...
    -- OLD size formula restored (mobileButtonsSize default=80 â†’ BTN_SIZE=44)
    ...
    -- Colors exactly matching photo
    local PINK_ON      = Color3.fromRGB(255,105,180)
    local BTN_BG_OFF   = Color3.fromRGB(46,18,36)       -- dark maroon card
    local PANEL_BG     = Color3.fromRGB(32,12,26)        -- even darker panel behind buttons
    local BORDER_OFF   = Color3.fromRGB(150,50,100)      -- dim pink border when off
    local BORDER_ON    = Color3.fromRGB(255,105,180)     -- bright pink border when on
    local TEXT_OFF     = Color3.fromRGB(255,190,215)     -- soft pink-white text off
    local TEXT_ON      = Color3.fromRGB(255,255,255)     -- white text on
    local STROKE_W     = math.max(2,math.floor(BTN_SIZE*0.05))

    -- Outer panel (visible dark background card â€” the big rounded rect in photo)
    ...
    for i,def in ipairs(buttons) do
        ...
        -- Per-button saved positions (absolute screen coords)
        local initXO,initYO
...
-- ============================================================
-- MAIN GUI â€” Blossom Hub style: horizontal tabs, section labels, bordered rows
-- ============================================================
local function buildGui()
    clearPersistentConns();createStealBar()

    -- Color palette (dark maroon base, pink accent â€” Blossom Hub style)
    local BG         = Color3.fromRGB(20, 10, 16)      -- main background (very dark maroon)
    local BG2        = Color3.fromRGB(28, 14, 22)      -- header background
    local ROW_BG     = Color3.fromRGB(30, 14, 24)      -- row background
    local ROW_BORDER = Color3.fromRGB(60, 30, 50)      -- row border
    local WHITE      = Color3.fromRGB(255, 255, 255)
    ...
    local SECT_LBL   = Color3.fromRGB(200, 150, 175)   -- section label color
    ...
    -- Fix rounded corners blending into body
    ...
    -- Ping / FPS label (right side of header)
    ...
    pingFpsLbl.BackgroundTransparency=1;pingFpsLbl.Text="-- ms Â· -- fps"
    ...
    task.spawn(function()
        ...
        while pingFpsLbl and pingFpsLbl.Parent do
            ...
            if now-t0>=0.5 then
                ...
                pingFpsLbl.Text=(ok and math.floor(ms+0.5) or "--").."ms Â· "..fps.."fps"
                frames=0;t0=now
            end
            task.wait()
        end
    ...
    closeBtn.Text="âˆ’";closeBtn.TextColor3=WHITE
    ...
    for _,name in ipairs(TABS) do
        ...
        -- Pink underline (shown when active)
    ...
    -- ===== ROW HELPERS â€” Blossom Hub style =====
    ...
    -- Value box (dark rounded box on right side, Blossom Hub style)
    ...
    local function mkToggleKB(parent, txt, kbEntry, onToggle, onKB)
        ...
        -- shift pill left of KB button â€” place pill at right edge, kb just inside
        -- Actually for this layout: label left, [kb btn] [pill] right
        pill:Destroy() -- rebuild properly below
        -- Rebuilt: kb at right-78 to right-8, pill at right-44 to right-10 â†’ let's offset
    ...
    local function mkActionRow(parent, txt, onActivate)
        ...
        btn.Text="â–¶";btn.TextColor3=WHITE;btn.Font=Enum.Font.GothamBlack;btn.TextSize=11
        btn.AutoButtonColor=false;btn.ZIndex=9
        Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
        btn.Activated:Connect(function()
    ...
    do
        ...
        modeValLbl.Size=UDim2.new(0,60,0,14);modeValLbl.Position=UDim2.new(0,10,0.5,-7)
        ...
        -- update label x pos since we have the main label already
        modeValLbl.Position=UDim2.new(0.45,0,0.5,-7)
        local clk=Instance.new("TextButton",pill);clk.Size=UDim2.new(1,0,1,0);clk.BackgroundTransparency=1;clk.Text="";clk.ZIndex=11
    ...
    setAntiRagVisual=mkToggle(pg,"Anti Ragdoll",function(on) antiRagdollEnabled=on;if on then startAntiRagdoll() else stopAntiRagdoll() end end)
    ...
    local introToggle = mkToggle(pg, "Intro Song", function(on)
        ...
        saveConfig()
        -- If turning off and sound is playing, stop it
